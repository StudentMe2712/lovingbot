...
logger = setup_logger("reminder_add_command")
data_instance = Data()
db = DatabaseManager()
reminders_module = RemindersModule(db, None)

async def reminder_add_command(update: Update, context):
    logger.info(f"reminder_add_command: {update.effective_user.id}")
    ...
    if user_status == UserStatus.NOT_ALLOWED:
        ...
        return
    await reminders_module.add_reminder(update, context) 